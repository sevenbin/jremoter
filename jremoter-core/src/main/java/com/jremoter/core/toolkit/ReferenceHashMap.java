/**   
 * @title: ReferenceHashMap.java 
 * @package com.jremoter.core.toolkit.ReferenceHashMap
 * @description: 代码来自SPRING
 * @author TangBin tb_bin@163.com
 * @date 2018年1月19日 下午12:32:07 
 * @version 1.0.0
 */
...
public class ReferenceHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
	...
	public boolean remove(Object key, final Object value) {
		return doTask(key, new Task<Boolean>(TaskOption.RESTRUCTURE_AFTER,TaskOption.SKIP_IF_EMPTY) {
			@Override
			protected Boolean execute(Reference<K, V> reference,
					Entry<K, V> entry) {
	...
	private Segment getSegmentForHash(int hash) {
		return this.segments[(hash >>> (32 - this.shift))
	...
	private abstract class Task<T> {
		...
		public Task(TaskOption... options) {
			this.options = (options.length == 0 ? EnumSet
					.noneOf(TaskOption.class) : EnumSet.of(options[0], options));
		}
	...
	private class EntrySet extends AbstractSet<Map.Entry<K, V>> {
		...
		public boolean contains(Object o) {
			if (o != null && o instanceof Map.Entry<?, ?>) {
				...
				Reference<K, V> reference = ReferenceHashMap.this.getReference(
		...
		public boolean remove(Object o) {
			if (o instanceof Map.Entry<?, ?>) {
				...
				return ReferenceHashMap.this.remove(entry.getKey(),
						entry.getValue());
			}
			return false;
		}
